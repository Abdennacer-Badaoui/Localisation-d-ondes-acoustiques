...
def _set_trifem_assembly(p_elem2nodes, elem2nodes, node_coords, f_unassembled):
    ...
    K = numpy.zeros((nnodes, nnodes), dtype=numpy.float64)
    M = numpy.zeros((nnodes, nnodes), dtype=numpy.float64)
    F = numpy.zeros((nnodes, 1), dtype=numpy.float64)
    for e in range(0, nelems):
        ...
        # area = numpy.linalg.norm(coords[0, :] - coords[1, :])
